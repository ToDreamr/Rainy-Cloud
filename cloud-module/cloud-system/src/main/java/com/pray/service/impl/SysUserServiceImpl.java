...
public class SysUserServiceImpl implements ISysUserService {
    ...
    public void checkUserAllowed(SysUser user) {
        if (StringUtils.isNotNull(user.getUserId()) && user.isAdmin()) {
            throw new CloudException("不允许操作超级管理员用户");
        }
    }
    ...
    public void checkUserDataScope(Long userId) {
        if (true) {//todo 管理员权限
            ...
            if (StringUtils.isEmpty(users)) {
                throw new CloudException("没有权限访问用户数据！");
            }
        }
    }
    ...
    public String importUser(List<SysUser> userList, Boolean isUpdateSupport, String operName) {
        if (StringUtils.isNull(userList) || userList.size() == 0) {
            throw new CloudException("导入用户数据不能为空！");
        }
        int successNum = 0;
        int failureNum = 0;
        StringBuilder successMsg = new StringBuilder();
        StringBuilder failureMsg = new StringBuilder();
        String password = "sys.user.initPassword";
        for (SysUser user : userList) {
            try {
                ...
                if (StringUtils.isNull(u)) {
                    ...
                    user.setPassword(password);//todo 明文加密
        ...
        if (failureNum > 0) {
            ...
            throw new CloudException(failureMsg.toString());
        } else {
