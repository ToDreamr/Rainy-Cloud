...
import com.pray.mapper.PreviousUserMapper;
...
/**
 * PreviousUserServiceImpl
 *
 * @author 春江花朝秋月夜
 * @since 2024/1/24 11:49
 */
@Service
public class PreviousUserServiceImpl extends ServiceImpl<PreviousUserMapper, Account> implements PreviousUserService {
    //便于测试，目前只通过数据库查数据
    @Resource
    PreviousUserMapper previousUserMapper;
    @Resource
    ...
    public List<Account> getUserList() {
        ...
        if (account ==null){
            try {
                ...
                queryWrapper.select();
                List<Account> accounts = previousUserMapper.selectList(queryWrapper);
